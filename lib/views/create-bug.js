...
var _ = require('../pouch-promise-cache');
...
function deleteAttachment(e) {
  bug.attachments = bug.attachments.filter(function(file) {
    return file.id != e.target.dataset.id;
  });
  bugChanged();
};
...
function drawAttachments() {
  tpl.readDom('#attachment-row-tpl').then(function(row) {
    ...
    bug.attachments.map(function(file, index) {
      if (!file.blob) {
        return;
      }
...
function submitBug(e) {
  ...
  var parts = value('#component').split('-');
  var product = parts.shift();
...
function showComponents() {
  ...
  _(app.bugzilla.getProducts.bind(app.bugzilla))({
    type: 'enterable',
    ...
    result.products.forEach(function(product) {
      product.components.forEach(function(component) {
        ...
        opt.value = product.name + ' - ' + component.name;
        frag.appendChild(opt);
      });
    });
    document.body.querySelector('#componentList').appendChild(frag);
    document.body.querySelector('#component').setAttribute('list', 'componentList');
  });
}
...
module.exports = function(ctx) {
  ...
  app = ctx.app;
  app.header('File a bug');

  return tpl.readDom('#create-bug-tpl').then(function(_form) {
    form = _form;
    ...
    form.querySelector('form')
      .addEventListener('input', bugChanged);
    form.querySelector('#createBug')
      .addEventListener('submit', submitBug.bind(self));
