...
function showAttachments(ctx) {

  return Promise.all([
    ctx.app.bugzilla.getAttachments(ctx.params.id),
    tpl.read('/views/view-attachment-row.tpl')
  ]).then(function(res) {

    var template = document.createElement('ul');
    template.classList.add('attachmentView');

    var attachments = res[0].bugs[ctx.params.id];
    ...
    attachments.forEach(function(attachment) {
      ...
      frag.appendChild(utils.render(res[1], {
...
module.exports = function(ctx) {
  ...
  return Promise.all([
    ...
    showAttachments(ctx).then(function(res) {
      if (res) {
        page.querySelector('#bugAttachments').appendChild(res);
      }
    });
